...
static RECEIVE_CANCEL: OnceLock<Mutex<Option<n0_future::task::AbortHandle>>> = OnceLock::new();
...
fn receive_cancel_slot() -> &'static Mutex<Option<n0_future::task::AbortHandle>> {
    RECEIVE_CANCEL.get_or_init(|| Mutex::new(None))
}
...
pub async fn receive_file(
    ...
    let task = n0_future::task::spawn(async move {
        let WasmReceiveResult { files } = download_files(ticket, options, app).await?;
        Ok::<_, anyhow::Error>(files)
    });
    *receive_cancel_slot().lock().expect("receive cancel mutex poisoned") = Some(task.abort_handle());

    let files = task.await.map_err(js_err)?.map_err(js_err)?;
    *receive_cancel_slot().lock().expect("receive cancel mutex poisoned") = None;
...
pub fn cancel_receive() {
    let mut guard = receive_cancel_slot().lock().expect("receive cancel mutex poisoned");
